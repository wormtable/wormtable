...
Drosophila genetic reference panel~\citep{mr+12}.
This VCF\footnote{\href%
{ftp://ftp.hgsc.bcm.edu/DGRP/freeze2\_Feb\_2013/vcf\_files/freeze2.vcf.gz}
{ftp://ftp.hgsc.bcm.edu/DGRP/freeze2\_Feb\_2013/vcf\_files/freeze2.vcf.gz}}
contains variant calls 
from a population consisting 192 inbred Drosophila lines and contains data 
from the whole genome over a total of 6,146,611 rows and consists
of 15GB of uncompressed text. The VCF was converted to 
wormtable format using the included \texttt{vcf2wt} program
on a standard workstation with a single hard disk. The
conversion required approximately 70 minutes, and the size of the resulting  
...
in the dataset, using wormtable, standard Unix tools and PyVCF\footnote{%
\href{http://pyvcf.rtfd.org/}{http://pyvcf.rtfd.org/}}, 
a Python VCF parser.
In each examples we proceed 
...
Using PyVCF this required approximately 130 minutes. The Unix tools 
(\texttt{cut}, \texttt{grep} and \texttt{perl}) method required 
1m17s, and wormtable required 55 seconds. Both the Unix tools 
and wormtable were limited by the speed of the hard disk in the
test machine, and therefore could be much faster using 
modern solid state 
storage. It should be noted, however, that this simple task represents 
