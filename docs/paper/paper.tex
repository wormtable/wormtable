...
% DAN: tabix is just for tables that have CHROM and POS values though right?
% This is a limitation that it might be worth pointing out. 
...
% DAN: what exactly does this mean - what makes it specific to biology??
...
precision floating point values. % DAN: as necessary? Or is this manually specified?
...
% I'm not sure its necessary to include the time to sort this - my guess is that
% most VCFs will be sorted, and for simplicity we could skip over this.
Afterwards,
...
% DAN: At the moment, a reader might conclude that if they have a standard
% hard disk, they should probably choose tabix rather than womrtable.
% It would be good to point out here what other benefits wormtable has over
% tabix. These would probably be: 1. flexbilility (not limitied to CHOM, POS
% indexed files) and the ability to index over other columns. 2. Similar 
% performance on a hard disk could be achieved by compressing the wormtable data
% 3. For VCFs and GTFs, information in compound columns can be parsed in advance 
% making access to it much faster - basically tabix only returns matching rows
% whereas wormtable provides parsed data for each row. Any other points??
...
unsustainable.
% DAN: Bindings can be less flexbile - for example tabix only returns 
% rows rather than fully parsed data. They might not allow sorting etc.
% Whereas a general solution could provide many more features?
...
%DAN: essentially any tabular data? What are the restrictions? This might be 
% a good time to bring it up - limited field size etc?
...
% I don't understand this point - isn't this what a cursor does
% You specify start and end and it gets all matching rows?
...
% DAN: I think the major problem is not being able to seek to random parts of the
% file, which is a major pain when the file is big.
