...
arguments, \texttt{start} and \texttt{stop}, which dictate 
...
is that accessing a particular row efficiently without some auxiliary 
index. Without some information on where a particular row is located
in a file, there is nothing that can be done except to read the file 
...
and indexed it using \texttt{tabix}. Since the VCF file was not 
...
To duplicate the functionality of Tabix in wormtable, we simply need to 
create an index on the CHROM and POS columns using \texttt{wtadmin add}. 
This required approximately
2 minutes and the resulting index consumed 115.4MB of space.
...
required.  On the other hand, though, decompression is expensive and must 
...
languages such as Perl, R and PHP. Compression of data can
...
using \texttt{wtadmin add} command, and can then efficiently
