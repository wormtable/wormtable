...
indexing on columns within these tables. 
        ...
        \paragraph*{Conclusions:} \emph{ Conclusions, brief summary and potential implications. }
\end{abstract}
...
floating point and character data. To allow data to be stored as 
efficiently as possible, wormtable supports a wide range of 
type sizes: integers range from one to eight bytes long,
and IEEE half, single and double precision floating point 
are supported. Fixed and variable length character strings 
are also supported.
...
wormtable uses Berkeley DB~\cite{obs99} for row storage and indexing.
...
world~\cite{sb12}. Berkeley DB is a highly modular library, providing the 
...
Each table corresponds to a single directory in the file system.
...
are there other formats we should chuck in here?]. Since the conversion 
tool can be written entirely in Python this is not a difficult task.

Other probable directions for future development of wormtable would
be to incorporate BGZF compression~\cite{li11}, allowing for smaller
files with efficient random access; to develop a 
...
to the community. 
Wormtable is an open and collaborative project, which would welcome 
