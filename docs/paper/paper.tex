...
Biological research, including genomics generates huge amounts of data.
The data produced by 
programs are commonly stored as tables in plain text files and are often very large. 
Researchers must process these files 
line-by-line, parsing values encoded as text into native types so that 
calculations may be done. This is an error prone and time consuming process.
...
much greater freedom to explore their data. Our comparisons with some 
alternative software demonstrates a 100-fold increase in speed. Additionally, 
because it is no longer necessary to parse complex files,
